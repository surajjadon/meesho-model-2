...
export const getPLSummary = async (req: Request, res: Response) => {
    try {
        ...
        // 1. Fetch Data
        const histories = await PaymentHistory.find({ businessGstin: gstin as string });
        
        // 2. Pre-fetch Mappings & Returns
        const skuMappings = await SkuMapping.find({ gstin: gstin as string });
