...
import { api } from "@/providers/GlobalProvider"; // ⚠️ Ensure this path matches your project
import { 
...
export default function InviteUserModal({ isOpen, onClose, gstList, initialData, onSave }: InviteUserModalProps) {
  ...
  const initialFormData = {
    ...
    gstAccess: { allFuture: true, selectedIds: [] as string[] }
  };
 ...
 // --- 🚀 THE IMPORTANT PART: REAL API CALL ---
  const handleSaveOrSend = async () => {
    ...
    // 1️⃣ Calculate the 'gstinvalue' array (list of strings)
    // We look at the selected IDs, find the matching object in gstList, and grab the .gstin string
    const calculatedGstinValues = gstList
      .filter((gst) => formData.gstAccess.selectedIds.includes(gst._id))
      .map((gst) => gst.gstin);

    // 2️⃣ Construct Payload matching your Mongoose Schema structure
    const payload = {
        id: initialData?.id, // Keep ID for edits
        name: formData.name,
        ...
        // Map 'gstAccess' -> Schema fields
        allowedGSTs: formData.gstAccess.selectedIds,
        gstAccessAll: formData.gstAccess.allFuture,

        // ✅ The specific field you requested
            ...
            <div className={`p-6 overflow-y-auto space-y-8 custom-scrollbar relative ${inviteStatus === 'sending' ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <div>
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <label className="flex items-center space-x-2 text-sm text-blue-600 cursor-pointer hover:text-blue-700 select-none">
                            <input type="checkbox" className="rounded text-blue-600 focus:ring-blue-500 cursor-pointer" checked={formData.gstAccess.allFuture} onChange={() => setFormData(prev => ({...prev, gstAccess: { ...prev.gstAccess, allFuture: !prev.gstAccess.allFuture }}))} />
                           
                        </label>
                    </div>
                    <div className={`space-y-2 max-h-40 overflow-y-auto ${formData.gstAccess.allFuture ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
