...
import { 
    Upload, IndianRupee, Calendar, CheckCircle, Activity, FileText, 
...
// --- Component: Payment Skeleton ---
const PaymentSkeleton = () => (
    <div className="w-full max-w-[1600px] mx-auto px-4 py-6 space-y-6 animate-pulse">
        {/* Header Bar */}
        <div className="bg-white p-6 rounded-xl border border-gray-200 flex justify-between items-center">
            ...
            <div className="flex gap-4">
                <div className="h-10 w-64 bg-gray-100 rounded border border-gray-200"></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-white p-5 rounded-xl border border-gray-200 h-32 flex flex-col justify-between">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Chart */}
            <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-200 h-[400px]">
                <div className="h-6 w-32 bg-gray-300 rounded mb-6"></div>
                <div className="h-[300px] bg-gray-50 rounded"></div>
            </div>
            {/* History List */}
            <div className="lg:col-span-1 bg-white rounded-xl border border-gray-200 h-[400px] p-6 space-y-4">
                <div className="flex justify-between mb-4">
                    <div className="h-6 w-24 bg-gray-300 rounded"></div>
                    <div className="h-4 w-4 bg-gray-300 rounded"></div>
                </div>
                {[1,2,3,4,5].map(j => (
                    <div key={j} className="h-14 w-full bg-gray-50 rounded border-b border-gray-100"></div>
                ))}
            </div>
        </div>
    </div>
...
// --- Component: Stat Card ---
const StatCard = ({ title, value, icon, highlight, loading }: { title: string; value: string; icon: React.ReactNode, highlight?: boolean, loading?: boolean }) => (
...
// --- Component: Detailed View (Refactored with useQuery) ---
const UploadDetailView = ({ 
    ...
    // --- QUERY: Fetch Paginated Details ---
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['paymentDetails', uploadId, currentPage, itemsPerPage, searchTerm],
        queryFn: async () => {
            const params = {
                page: currentPage,
                limit: itemsPerPage,
            ...
            const res = await api.get(`/payments/history/${uploadId}/paginated`, { params });
            return res.data;
        },
    ...
    const pageData = data?.rawOrderPayments || [];
    const meta = { 
    ...
    return (
        <div className="bg-white w-full rounded-xl border border-gray-200 shadow-lg flex flex-col h-[700px] overflow-hidden animate-in slide-in-from-right duration-300">
...
export default function PaymentsDashboard() {
    ...
    const [confirmReplaceData, setConfirmReplaceData] = useState<{ isOpen: boolean; message: string; file: File | null } | null>(null);
    const [viewDetailsId, setViewDetailsId] = useState<string | null>(null);

    // 1️⃣ FIX: Business Guard Ref
    ...
    // 2️⃣ FIX: Sync Ref with Selected Business
    useEffect(() => {
        if (!selectedBusiness?.gstin) return;
        activeGstinRef.current = selectedBusiness.gstin;
    }, [selectedBusiness?.gstin]);

    // --- 1. Queries ---

    const { data: stats = ZERO_STATS, isLoading: statsLoading } = useQuery({
        queryKey: ['paymentStats', selectedBusiness?.gstin, startDate, endDate],
        queryFn: async () => {
            const res = await api.get('/payments/stats', { 
                params: { gstin: selectedBusiness?.gstin, startDate, endDate } 
            });
            return res.data as PaymentStats;
        },
        enabled: !!selectedBusiness?.gstin
    });

    const { data: histories = [] } = useQuery({
        queryKey: ['paymentHistory', selectedBusiness?.gstin],
        queryFn: async () => {
    ...
    const { data: trendData = [], isLoading: trendLoading } = useQuery({
        queryKey: ['paymentTrend', selectedBusiness?.gstin, startDate, endDate],
        queryFn: async () => {
            const res = await api.get('/payments/trend', { 
                params: { gstin: selectedBusiness?.gstin, startDate, endDate } 
            });
    ...
    // --- 2. Mutation: Upload File ---
    const uploadMutation = useMutation({
        mutationFn: async ({ fileToUpload, confirmReplace }: { fileToUpload: File, confirmReplace?: boolean }) => {
        ...
        onSuccess: () => {
            // 3️⃣ FIX: Only update if business hasn't changed
            if (activeGstinRef.current !== selectedBusiness?.gstin) return;

            queryClient.invalidateQueries({ queryKey: ['paymentStats'] });
            queryClient.invalidateQueries({ queryKey: ['paymentHistory'] });
            queryClient.invalidateQueries({ queryKey: ['paymentTrend'] });
            
            setFile(null);
            setConfirmReplaceData(null);
        },
        onError: (err: any, variables) => {
            if (activeGstinRef.current !== selectedBusiness?.gstin) return;

            if (err.response && err.response.status === 409) {
                setConfirmReplaceData({
                    isOpen: true,
                    message: err.response.data.message,
                    file: variables.fileToUpload
                });
            } else {
                handleApiError(err, "Upload failed. Check file format.");
            }
        }
    });
    ...
    const handleProcessFile = (isRetry = false) => {
        const fileToUpload = isRetry ? confirmReplaceData?.file : file;
        if (!fileToUpload || !selectedBusiness) return;
        uploadMutation.mutate({ fileToUpload, confirmReplace: isRetry });
    };
    ...
    if (isLoading && !stats.totalNetOrderAmount) return <PaymentSkeleton />;

    return (
        <div className="w-full max-w-[1600px] mx-auto px-4 py-6 space-y-6 font-sans animate-in fade-in duration-500">
                ...
                <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                        ...
                        <div className="flex gap-2">
                            <button 
                                ...
                                disabled={isLoading}
                                className="cursor-pointer px-4 py-2.5 bg-indigo-50 text-indigo-700 text-sm font-medium rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-100 min-w-[80px] flex items-center justify-center"
                            >
                                {isLoading ? <Loader2 className="animate-spin h-4 w-4"/> : "Apply"}
                            </button>
                            <button 
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-4">
                        ...
                        {file && (
                            ...
                            >
                                {uploadMutation.isPending ? <RefreshCw className="animate-spin" size={16} /> : "Import"}
                            </button>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <StatCard 
                    ...
                    value={formatCurrency(stats.totalNetOrderAmount)} 
                ...
                <StatCard 
                    title="Ads Deduction" 
                    value={formatCurrency(stats.totalAdsCost)} 
                    icon={<Activity size={20} />} 
                    loading={isLoading}
                />
                <StatCard 
                    title="Payments Count" 
                    value={stats.paymentsCount.toLocaleString()} 
            ...
            {confirmReplaceData && confirmReplaceData.isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4 border border-gray-100 animate-in fade-in zoom-in duration-200">
                        <div className="flex items-center gap-3 text-amber-600 mb-4">
                            <AlertCircle size={28} />
                            <h3 className="text-lg font-bold text-gray-900">Duplicate Data Detected</h3>
                        </div>
                        
                        <p className="text-gray-600 mb-6 leading-relaxed">
                            {confirmReplaceData.message}
                            <br/><br/>
                            Do you want to <span className="font-bold text-gray-800">replace</span> the existing data with this new file? This will overwrite the previous entries.
                        </p>
                        
                        <div className="flex justify-end gap-3">
                            <button 
                                onClick={() => setConfirmReplaceData(null)}
                            ...
                            <button 
                                ...
                                className="px-4 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg font-medium shadow-md transition-colors flex items-center gap-2"
                            >
                                <RefreshCw size={16} /> Replace Data
                            </button>
                        </div>
                    </div>
                </div>
