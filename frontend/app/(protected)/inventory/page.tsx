...
const HistoryModal = ({
  ...
  const getReasonIcon = (reason: IStockHistory["reason"]) => {
    switch (reason) {
      ...
      case "Initial Stock":
        return <PackagePlus size={16} className="text-green-500" />;
      default:
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-8 relative flex flex-col max-h-[90vh]">
        ...
        <div className="flex-shrink-0">
          ...
          <p className="text-slate-600 mb-6 font-medium">{title}</p>
        </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-slate-100">
                <tr>
                  <th className="p-3 text-left font-semibold text-slate-600 uppercase">
                    Date
                  </th>
                  <th className="p-3 text-left font-semibold text-slate-600 uppercase">
                    Reason
                  </th>
                  <th className="p-3 text-center font-semibold text-slate-600 uppercase">
                    Change
                  </th>
                  <th className="p-3 text-left font-semibold text-slate-600 uppercase">
                    Stock Level
...
// ----------------- Add Item Modal -----------------
// IMPORTANT: modal does NOT post to API. It returns form fields + file via onCreate
const AddItemModal = ({
  isOpen,
  onClose,
  onCreate,
  ...
  // Modal returns the raw form data to parent; parent will add gstin & post
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-lg">
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose} className="bg-slate-200 px-4 py-2 rounded">
            Cancel
          </button>
          <button
...
export default function InventoryPage() {
  ...
  const [itemHistory, setItemHistory] = useState<IStockHistory[]>([]);
  ...
  // --- Input helpers ---
  const handleDirectStockInput = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAddStockInput = (e: ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    if (
      value === "" ||
      (!isNaN(parseInt(value, 10)) && parseInt(value, 10) > 0)
    ) {
      setAddingStock({ ...addingStock, value });
    }
  };
  ...
  const handleStockUpdate = async (itemId: string) => {
    ...
    if (isNaN(newStock) || newStock === originalItem?.stock || newStock < 0) {
    ...
    try {
      ...
      setInventory((prev) => prev.map((it) => (it._id === itemId ? data : it)));
    } catch (err: any) {
      console.error(err);
  ...
  const handleAddStock = async (itemId: string) => {
    const originalItem = inventory.find((i) => i._id === itemId);
    const addAmount = parseInt(addingStock.value, 10);
    if (isNaN(addAmount) || addAmount <= 0) {
      setAddingStock({ id: null, value: "" });
      return;
    }
    const newStock = (originalItem?.stock ?? 0) + addAmount;
    try {
      const { data } = await api.put(`/inventory/${itemId}`, {
        stock: newStock,
      });
  ...
  const handleStockKeyDown = (
    ...
    if (e.key === "Enter") handleStockUpdate(itemId);
    else if (e.key === "Escape") setEditingStock({ id: null, value: "" });
  };

  const handleAddStockKeyDown = (
    e: KeyboardEvent<HTMLInputElement>,
    itemId: string
  ) => {
    if (e.key === "Enter") handleAddStock(itemId);
    else if (e.key === "Escape") setAddingStock({ id: null, value: "" });
  };

  const stats = useMemo(
    () => ({
      totalSKUs: inventory.length,
      unitsInStock: inventory.reduce((sum, item) => sum + (item.stock || 0), 0),
      lowStockSKUs: inventory.filter(
        (item) =>
          (item.stock || 0) > 0 && (item.stock || 0) <= lowStockThreshold
      ).length,
      outOfStockSKUs: inventory.filter((item) => (item.stock || 0) <= 0).length,
    }),
  ...
  const categories = useMemo(
    () => [
      "All",
      ...Array.from(
        new Set(inventory.map((i) => i.category).filter(Boolean) as string[])
      ),
    ],
    [inventory]
  );
    ...
    return inventory
      .filter((item) => {
        if (activeTab === "Low stock")
          return getStatus(item).key === "low-stock";
        if (activeTab === "Out of stock")
          return getStatus(item).key === "out-of-stock";
        return true;
      })
      .filter(
        (item) => statusFilter === "All" || getStatus(item).key === statusFilter
      )
      .filter(
  ...
  // ----------------- History fetch -----------------
  const handleViewHistory = (item: IInventoryItem) => {
    setSelectedItemTitle(item.title);
    setHistoryModalOpen(true);
    setHistoryLoading(true);
    api
      .get(`/inventory/${item._id}/history`)
      .then((res) => setItemHistory(res.data || []))
      .catch((err) => {
  ...
  // ----------------- Add item flow (parent handles API) -----------------
  ...
  const handleCreateItem = async (itemData: any) => {
    ...
    try {
      ...
      // Append GSTIN exactly once
      formData.append("gstin", selectedBusiness.gstin);

      // Append normal fields (skip gstin, null/undefined, empty strings, and featuredImageFile)
      ...
      // Append file if present - MUST match backend field name "featuredImage"
      if (itemData.featuredImageFile) {
        formData.append("featuredImage", itemData.featuredImageFile); // ← Changed from "featuredImageFile"
      }
  ...
  return (
    <div className="bg-slate-100 min-h-screen w-full p-6 lg:p-8 space-y-6">
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200/80 flex flex-wrap items-center gap-4 text-sm">
        ...
        <div className="relative">
          <select
            ...
            value={statusFilter}
            className="appearance-none bg-slate-100/80 border-none rounded-md py-2 pl-3 pr-8 focus:ring-2 focus:ring-blue-500 cursor-pointer"
          >
        ...
        <div className="lg:col-span-2">
          <RestockSuggestions
            items={inventory.filter(
              (item) => getStatus(item).key === "low-stock"
            )}
                      ...
                      <div className="flex items-center gap-2">
                        {editingStock.id === item._id ? (
                          <input
                            ...
                            onChange={handleDirectStockInput}
                            onBlur={() => handleStockUpdate(item._id)}
                            onKeyDown={(e) => handleStockKeyDown(e, item._id)}
                            className="w-24 p-1 border rounded-md border-blue-400"
                            autoFocus
                            placeholder="Set stock"
                          />
                        ) : addingStock.id === item._id ? (
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              type="number"
                              min={1}
                              value={addingStock.value}
                              onChange={handleAddStockInput}
                              onBlur={() => handleAddStock(item._id)}
                              onKeyDown={(e) =>
                                handleAddStockKeyDown(e, item._id)
                              }
                              className="w-20 p-1 border rounded-md border-green-400 text-center"
                              autoFocus
                              placeholder="Add qty"
                            />
                            {addingStock.value &&
                              parseInt(addingStock.value, 10) > 0 && (
                                <span className="text-slate-500 text-sm">
                                  ={" "}
                                  <span className="font-bold text-green-600">
                                    {(item.stock ?? 0) +
                                      parseInt(addingStock.value, 10)}
                                  </span>
                                </span>
                              )}
                          </div>
                          ...
                          <>
                            <div
                              ...
                              className="p-1 rounded-md hover:bg-slate-200 min-w-[40px] cursor-pointer"
                            ...
                            {hoveredRow === item._id && (
                              <button
                                onClick={() =>
                                  setAddingStock({ id: item._id, value: "" })
                                }
                                className="flex items-center gap-1 px-2 py-1 text-xs font-semibold text-green-700 bg-green-100 hover:bg-green-200 rounded-md transition-all whitespace-nowrap"
                                title="Add to stock"
                              >
                                <Plus size={14} /> Add
                              </button>
      ...
      {/* Add Item Modal (modal returns fields; parent handles POST & gstin) */}
      <AddItemModal
        isOpen={addModalOpen}
