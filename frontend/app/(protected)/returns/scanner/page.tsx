...
export default function ScannerPage() {
  ...
  const [selectedIds, setSelectedIds] = useState<string[]>([]); // Selected _IDs
  ...
  // --- 2. MUTATION: Verify Returns (Single & Bulk) ---
  const verifyMutation = useMutation({
    ...
    onMutate: async ({ ids, status }) => {
        ...
        // We need to map _id (UI selection) back to subOrderNo (API requirement) OR just find by whatever logic matches
        // The mutation sends `ids` which are subOrderNos based on usage below.
    ...
    onSettled: () => {
        ...
        // Refocus input after any operation
  ...
  const handleBulkUpdate = async () => {
    ...
    // Map selected _ids (UI) to subOrderNos (API)
