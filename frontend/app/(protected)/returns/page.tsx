...
export default function ScannerPage() {
  ...
  const [targetStatus, setTargetStatus] = useState<"None" | "Delivered" | "Cancelled" | "Return" | "RTO" | "Undelivered" | "Damaged">("Return");
  ...
  const { 
    ...
    queryKey: ['returns', selectedBusiness?.gstin],
    queryFn: async () => {
        const res = await api.get("/returns", { params: { gstin: selectedBusiness?.gstin } });
        return res.data as IReturnOrder[];
    },
  ...
  // --- 2. MUTATION: Verify Returns ---
  const verifyMutation = useMutation({
    mutationFn: async ({ ids, status }: { ids: string[], status: string }) => {
        return api.post("/returns/verify", { 
    ...
    onMutate: async ({ ids, status }) => {
        await queryClient.cancelQueries({ queryKey: ['returns', selectedBusiness?.gstin] });
        const previousReturns = queryClient.getQueryData<IReturnOrder[]>(['returns', selectedBusiness?.gstin]);

        queryClient.setQueryData<IReturnOrder[]>(['returns', selectedBusiness?.gstin], (old) => {
            if (!old) return [];
            return old.map(order => 
                ids.includes(order.subOrderNo) 
                ? { ...order, verificationStatus: status as any, updatedAt: new Date().toISOString() } 
                : order
    ...
    onError: (err, variables, context) => {
        queryClient.setQueryData(['returns', selectedBusiness?.gstin], context?.previousReturns);
        toast.error("Update Failed", { description: "Failed to update orders." });
    },
    onSettled: () => {
        queryClient.invalidateQueries({ queryKey: ['returns', selectedBusiness?.gstin] });
  ...
  const handleBulkUpdate = async () => {
    ...
    verifyMutation.mutate({ ids: idsToSend, status: targetStatus }, {
        onSuccess: () => {
            setSelectedIds([]);
            toast.success("Orders Updated", { description: `Updated ${idsToSend.length} orders to '${targetStatus}'.` });
        }
    });
  };
  ...
  const handleScan = () => {
    ...
    if (!item) {
      toast.error("Item Not Found", { description: `Input: "${input}"` });
      setScanInput("");
      return;
    }

    verifyMutation.mutate({ ids: [item.subOrderNo], status: targetStatus }, {
        onSuccess: () => {
            setScanInput("");
            toast.success("Verified", { description: `${targetStatus}: ${item?.subOrderNo}` });
        }
    });
  };
  ...
  return (
    ...
    <div className="space-y-6 w-full min-h-screen bg-slate-50 p-4 md:p-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-3">
            <button 
                onClick={() => refetch()} 
                disabled={loading}
                className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg font-medium transition-colors disabled:opacity-50"
            >
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500">
                  {verifyMutation.isPending && !selectedIds.length ? <Loader2 className="animate-spin" size={24} /> : <ScanBarcode size={24} />}
              </div>
              <input 
                ref={scanInputRef} 
                type="text" 
                ...
                disabled={verifyMutation.isPending && !selectedIds.length}
                placeholder="Click here and scan barcode..." 
                className="w-full pl-14 pr-4 py-4 text-lg font-mono bg-blue-50/30 border-2 border-blue-500/30 focus:border-blue-600 rounded-xl focus:ring-4 focus:ring-blue-100 outline-none transition-all placeholder:text-slate-400 text-slate-800 disabled:opacity-70 disabled:cursor-not-allowed" 
              />
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                // --- Reverted to bg-gray-300 skeletons ---
                Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                        <td className="p-4"><div className="w-4 h-4 bg-gray-300 rounded"></div></td>
                        ...
                        <td className="p-4"><div className="h-6 w-16 bg-gray-300 rounded-full"></div></td>
                        <td className="p-4"><div className="h-3 w-24 bg-gray-300 rounded"></div></td>
                    </tr>
                ))
